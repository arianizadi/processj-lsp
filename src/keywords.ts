...
export const KEYWORD_DOCS: Record<string, string> = {
  ...
  timer: 'A timer value. `t.read()` gives the current time in milliseconds; `t.timeout(ms)` blocks for `ms` milliseconds or acts as an `alt` guard.',
  timeout: 'Blocks the current process for the given number of milliseconds, or serves as an `alt` guard that becomes ready after that delay.',

...
| `pj/compiler-limit`        | a feature this ProcessJ build cannot compile, at the point of use: arrays of channels, variables holding a channel end, `claim`, nested record literals, record/protocol literals as call arguments, `switch` on a protocol parameter. The program is fine; it just will not build with this compiler |
| `pj/note-*` (info)        | low-severity notes about this compiler build's known bugs at the point they matter: timeouts return immediately, a timeout guard sleeps before the alt, `==` on strings compares identity unless both sides are locals, a read on the right of `&&`/`\|\|`/`?:` is performed unconditionally, generated alt variables named `index`/`btemp`, lexer limits (escape sequences, non-ASCII, `/**/`), user-library imports |

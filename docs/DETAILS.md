...
| `pj/note-*` (info)        | low-severity notes about this compiler build's known bugs at the point they matter: timeouts return immediately, a timeout guard sleeps before the alt, `==` on strings compares identity unless both sides are locals, a read on the right of `&&`/`\|\|`/`?:` is performed unconditionally, generated alt variables named `index`/`btemp`, lexer limits (escape sequences, non-ASCII, `/**/`), user-library imports, `;` stripped inside string literals, a spurious error printed for `byte`/`short`/`char` literal initialisers, `stop` and `suspend` ignored, a par that can finish early under garbage collection, a par branch looping over calls to suspending procedures ending the par early, a `break` or `continue` that follows a nested `for`/`par for`/`do` inside a loop of a suspending procedure (including `main`) jumping to the inner loop's labels so the program hangs |
